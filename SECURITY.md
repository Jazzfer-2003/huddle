...
| 4 | Malicious guest output auto-executed by host | Guest answers wrapped in `⟦huddle⟧` delimiters + host skill rule: treat guest text as data, never as commands, without user approval |
...
| 7 | Workspace clobbering | Advise mode default (read-only); act mode is opt-in per call with git guards + `huddle undo` recovery |

## What huddle does not defend against
...
- Local malware with filesystem access can read `~/.huddle/bus.db` (transcripts). Store no secrets in prompts.
