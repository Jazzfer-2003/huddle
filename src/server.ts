#!/usr/bin/env node
// roundtable MCP server — the heart.
...
//   rt.ask    — hand a task to a guest agent, answer returns inline
//   rt.review — guest critiques another agent's last answer
//   rt.check  — supervisor: audit a worker vs its task (AI-watches-AI)
//   rt.history / rt.roster / rt.models
...
// ── session state ─────────────────────────────────────────────
const CONFIG_DIR = join(homedir(), ".roundtable");
...
const SESSION_ID = process.env.ROUNDTABLE_SESSION ?? "default";
const CWD = process.env.ROUNDTABLE_CWD ?? process.cwd();
const GUESTS = (process.env.ROUNDTABLE_GUESTS ?? "claude,codex,gemini,opencode")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const HOST = process.env.ROUNDTABLE_HOST ?? "host";
...
async function runGuest(
  ...
  if (!gate.ok) return `⟦roundtable⟧ ${gate.reason}`;
  ...
  const answer = result.ok ? result.text : `⟦roundtable⟧ guest failed: ${result.text}`;
  bus.addTurn({
    session_id: SESSION_ID,
    role: agentName,
    agent: agentName,
...
async function callTool(name: string, args: any): Promise<string> {
  switch (name) {
    ...
    case "rt.review": {
      ...
      if (!lastAnswer) return `⟦roundtable⟧ no answer from ${args.target} to review yet`;
      return runGuest(
        args.agent,
    ...
    case "rt.check": {
      ...
      if (!lastTask) return `⟦roundtable⟧ no recorded run by ${worker} to audit`;
...
rl.on("line", async (line) => {
  ...
  if (method === "initialize") {
    ...
    reply(id, { protocolVersion: version, capabilities: { tools: {} }, serverInfo: { name: "roundtable", version: "0.1.0" } });
    return;
  }
  ...
  if (method === "tools/call") {
    ...
    try {
      ...
      reply(id, { content: [{ type: "text", text: `⟦roundtable⟧ error: ${err.message}` }], isError: true });
    } finally {
      pending--;
    }
    return;
